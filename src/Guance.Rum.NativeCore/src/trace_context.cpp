...
void initialize_c_context(guance_rum_trace_context& context) {
...
std::optional<TraceContext> custom_context(
    ...
    guance_rum_trace_context supplied{};
    ...
    constexpr std::size_t required_size =
        offsetof(guance_rum_trace_context, headers) + sizeof(supplied.headers);
    if (provided == 0 ||
        supplied.struct_size < required_size ||
        supplied.version != GUANCE_RUM_TRACE_CONTEXT_VERSION ||
        supplied.header_count > GUANCE_RUM_TRACE_MAX_HEADERS) {
        return std::nullopt;
    }
...
bool valid_trace_type(guance_rum_trace_type value) {
    return value >= GUANCE_RUM_TRACE_DDTRACE && value <= GUANCE_RUM_TRACE_JAEGER;
}
...
bool trace_config_from_c(
    const guance_rum_trace_config& source,
    const std::string& default_service_name,
    TraceConfig& destination) {
    constexpr std::size_t required_size =
        offsetof(guance_rum_trace_config, user_data) + sizeof(source.user_data);
    if (source.struct_size < required_size ||
        source.version != GUANCE_RUM_TRACE_CONFIG_VERSION ||
...
std::optional<TraceContext> create_trace_context(
    ...
    try {
        ...
        if (config.trace_type == GUANCE_RUM_TRACE_DDTRACE) {
        ...
        case GUANCE_RUM_TRACE_ZIPKIN_MULTI_HEADER:
            return make_context(
                is_sampled,
                config.enable_link_rum_data,
                trace_id,
        ...
        case GUANCE_RUM_TRACE_ZIPKIN_SINGLE_HEADER:
            return make_context(
                is_sampled,
                config.enable_link_rum_data,
                trace_id,
        ...
        case GUANCE_RUM_TRACE_TRACEPARENT:
            return make_context(
                is_sampled,
                config.enable_link_rum_data,
                trace_id,
        ...
        case GUANCE_RUM_TRACE_SKYWALKING: {
...
bool trace_context_to_c(
    ...
    guance_rum_trace_context& destination) noexcept {
    try {
        if (source.headers.size() > GUANCE_RUM_TRACE_MAX_HEADERS) {
            return false;
        }
        guance_rum_trace_context output{};
