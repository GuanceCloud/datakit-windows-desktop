...
std::optional<NativeMonitoringConfig> from_c_native_monitoring_config(
    ...
    if (config.struct_size < sizeof(guance_rum_native_monitoring_config) ||
...
public:
    ...
    bool enable(const guance_rum_native_monitoring_config& c_config) {
...
bool NativeMonitoring::enable(const guance_rum_native_monitoring_config& config) {
