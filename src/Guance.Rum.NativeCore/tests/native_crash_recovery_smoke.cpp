...
int child_main(const std::wstring& mode, const std::filesystem::path& crash_directory) {
    ...
    const auto queue_path = (crash_directory / "rum.db").string();
...
int64_t recover_default_on_init(const std::filesystem::path& root_directory) {
    const auto queue_path = (root_directory / "rum.db").string();
    guance_rum_config core_config{};
    guance_rum_config_init(&core_config);
    core_config.rum_app_id = "native-crash-parent";
    core_config.sample_rate = 1.0;
...
bool handlers_are_restored(const std::filesystem::path& crash_directory) {
    const auto queue_path = (crash_directory / "restore-rum.db").string();
    guance_rum_config core_config{};
    guance_rum_config_init(&core_config);
    ...
    monitoring.enable_native_crash_reporting = 1;
    monitoring.crash_cache_path = path.c_str();
    const bool enabled = guance_rum_enable_native_monitoring(handle, &monitoring) == 1;
    std::thread concurrent_capture([] { guance_rum_capture_cpp_terminate(); });
    guance_rum_disable_native_monitoring(handle);
