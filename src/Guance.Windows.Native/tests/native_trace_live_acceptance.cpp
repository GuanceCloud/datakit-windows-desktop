...
public:
    explicit RumHandle(guance_sdk_handle value) noexcept : value_(value) {}
    ~RumHandle() {
        if (value_ != nullptr) {
            guance_sdk_shutdown(value_);
        }
    }
    RumHandle(const RumHandle&) = delete;
    RumHandle& operator=(const RumHandle&) = delete;
    [[nodiscard]] guance_sdk_handle get() const noexcept { return value_; }

private:
    guance_sdk_handle value_ = nullptr;
};
...
int main() {
    ...
    try {
        const auto target = required_environment("GUANCE_TRACE_TEST_URL");
        ...
        config.debug = 1;
        RumHandle rum(guance_sdk_init(&config));
        ...
        trace.trace_type = GUANCE_TRACE_DDTRACE;
        require(
            guance_trace_configure(rum.get(), &trace) == 1,
        ...
        guance_sdk_flush(rum.get());
        guance_sdk_diagnostics diagnostics{};
        require(
            guance_sdk_get_diagnostics(rum.get(), &diagnostics) == 1,
