...
int main() {
    ...
    assert(monitoring.version == GUANCE_SDK_NATIVE_MONITORING_CONFIG_VERSION);
    ...
    const int null_handle_result = guance_sdk_enable_native_monitoring(nullptr, &monitoring);
    assert(null_handle_result == 0);
    const int null_config_result = guance_sdk_enable_native_monitoring(handle, nullptr);
    ...
    int invalid_result = guance_sdk_enable_native_monitoring(handle, &invalid);
    assert(invalid_result == 0);

    invalid = monitoring;
    invalid.version++;
    invalid_result = guance_sdk_enable_native_monitoring(handle, &invalid);
    assert(invalid_result == 0);
    ...
    const int enabled = guance_sdk_enable_native_monitoring(handle, &monitoring);
    assert(enabled == 1);
    const int enabled_again = guance_sdk_enable_native_monitoring(handle, &monitoring);
