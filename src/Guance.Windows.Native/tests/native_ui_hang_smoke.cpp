...
bool pump_until_event_count(guance_sdk_handle handle, int64_t expected, int timeout_ms) {
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        ...
        if (guance_sdk_get_diagnostics(handle, &diagnostics) &&
...
bool run_block(HWND window, guance_sdk_handle handle, DWORD duration_ms, int64_t expected_count) {
...
int main() {
    ...
    if (child_window == nullptr ||
        guance_sdk_enable_native_monitoring(handle, &invalid_monitoring)) {
    ...
    if (!guance_sdk_enable_native_monitoring(handle, &monitoring) ||
        !guance_sdk_enable_native_monitoring(handle, &monitoring)) {
        guance_sdk_shutdown(handle);
        DestroyWindow(window);
        return 5;
    }
    ...
    guance_sdk_diagnostics after_destroy{};
    guance_sdk_get_diagnostics(handle, &after_destroy);
