...
struct NativeDiagnostics {
    ...
    int64_t cache_allocated_bytes = 0;
...
struct LogConfig {
    ...
    Tags global_context;
    guance_log_discard_strategy discard_strategy = GUANCE_LOG_DISCARD_NEW;
};
...
private:
    ...
    bool consume_upload_budget(int64_t payload_bytes);
    ...
    std::chrono::steady_clock::time_point upload_budget_updated_at_ = std::chrono::steady_clock::now();
    double upload_byte_tokens_ = 0;
    double upload_request_tokens_ = 0;
    std::size_t upload_schedule_cursor_ = 0;
    std::array<std::chrono::steady_clock::time_point, 3> upload_retry_not_before_{};
    std::array<int, 3> upload_retry_attempts_{};
};

Config from_c_config(const guance_sdk_config* config);
