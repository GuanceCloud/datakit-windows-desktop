...
typedef struct guance_sdk_config {
    ...
    int64_t max_upload_bytes_per_second;
    int64_t upload_burst_bytes;
    double max_upload_requests_per_second;
    int max_upload_batches_per_cycle;
    int http_timeout_ms;
...
typedef enum guance_trace_type {
    GUANCE_TRACE_DDTRACE = 0,
    GUANCE_TRACE_ZIPKIN_MULTI_HEADER = 1,
    GUANCE_TRACE_ZIPKIN_SINGLE_HEADER = 2,
    GUANCE_TRACE_TRACEPARENT = 3,
    GUANCE_TRACE_SKYWALKING = 4,
    GUANCE_TRACE_JAEGER = 5
} guance_trace_type;

typedef struct guance_trace_header {
    char name[GUANCE_TRACE_HEADER_NAME_CAPACITY];
    char value[GUANCE_TRACE_HEADER_VALUE_CAPACITY];
} guance_trace_header;
...
typedef struct guance_trace_context {
    ...
    int sampled;
    int link_rum_data;
    char trace_id[GUANCE_TRACE_ID_CAPACITY];
    char span_id[GUANCE_TRACE_ID_CAPACITY];
    uint32_t header_count;
    guance_trace_header headers[GUANCE_TRACE_MAX_HEADERS];
...
typedef struct guance_trace_config {
    ...
    double sample_rate;
    guance_trace_type trace_type;
    const char* service_name;
    guance_trace_should_trace_callback should_trace;
    guance_trace_context_provider_callback context_provider;
    void* user_data;
} guance_trace_config;
...
typedef enum guance_log_level {
    GUANCE_LOG_DEBUG = 1u << 0,
    GUANCE_LOG_INFO = 1u << 1,
    GUANCE_LOG_WARNING = 1u << 2,
    GUANCE_LOG_ERROR = 1u << 3,
    GUANCE_LOG_CRITICAL = 1u << 4,
    GUANCE_LOG_OK = 1u << 5
} guance_log_level;

typedef enum guance_log_discard_strategy {
    GUANCE_LOG_DISCARD_NEW = 0,
    GUANCE_LOG_DISCARD_OLDEST = 1
...
typedef struct guance_log_config {
    ...
    const guance_log_property* global_context;
    uint32_t global_context_count;
    guance_log_discard_strategy discard_strategy;
} guance_log_config;
