...
$env:GUANCE_RUM_NATIVE_SESSION_REPLAY_ENABLED = if (
    $env:GUANCE_RUM_SESSION_REPLAY_ENABLED -match "^(1|true)$"
) { "1" } else { "0" }
$env:GUANCE_RUM_NATIVE_SESSION_REPLAY_SAMPLE_RATE = "1"
$env:GUANCE_RUM_NATIVE_REPLAY_PRIVACY_LEVEL = if ($env:GUANCE_RUM_REPLAY_PRIVACY_LEVEL) {
    $env:GUANCE_RUM_REPLAY_PRIVACY_LEVEL
} else {
    "mask"
}
