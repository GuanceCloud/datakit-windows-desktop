...
if (!rum || !window.FTWebViewJavascriptBridge) {
  ...
  if (replayEnabled && typeof rum.startSessionReplayRecording === "function") {
    rum.startSessionReplayRecording();
  }
  status.textContent = replayEnabled
    ? "RUM and experimental Session Replay initialized through the Windows native bridge."
    : "RUM initialized through the Windows native bridge.";
  status.dataset.state = "ready";

  actionButton.addEventListener("click", () => {
    rum.addAction("preview_button_click", { sample: "electron-mixed-preview" });
