...
public sealed class DatawayTransportTests
{
    ...
    public async Task SendAsync_RetriesServerErrorsButDropsClientErrors()
    {
        ...
        using var retryTransport = new DatawayTransport(new GuanceConfig
        {
        ...
        using var dropTransport = new DatawayTransport(new GuanceConfig
        {
            DatakitUrl = "http://127.0.0.1:9529",
            RumAppId = "app",
    ...
    [Fact]
    public async Task SendAsync_RetriesRateLimitsAndPreservesRetryAfter()
    {
        ...
        var result = await transport.SendAsync(
            new[] { new QueuedRumEvent(1, "x y=1i 1\n", DateTimeOffset.UtcNow) },
            CancellationToken.None);

        Assert.True(result.RetryLater);
        Assert.False(result.DeleteFromQueue);
    ...
    {
        using var timeoutTransport = new DatawayTransport(new GuanceConfig
        {
            DatakitUrl = "http://127.0.0.1:9529",
            RumAppId = "app",
        ...
        using var networkTransport = new DatawayTransport(new GuanceConfig
        {
            DatakitUrl = "http://127.0.0.1:9529",
            RumAppId = "app",
