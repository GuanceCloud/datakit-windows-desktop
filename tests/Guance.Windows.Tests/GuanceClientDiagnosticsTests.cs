...
public sealed class GuanceClientDiagnosticsTests
{
    [Fact]
    ...
    [Fact]
    public void GuanceConfig_RejectsUnsupportedEnvironment()
    {
        var exception = Assert.Throws<InvalidOperationException>(() => new GuanceConfig
        {
            DatakitUrl = "http://127.0.0.1:9529",
            RumAppId = "app",
            Env = "dev"
    ...
    [Fact]
    public void GuanceConfig_RejectsACacheThatCannotFitOneReplayBatch()
    {
        var exception = Assert.Throws<InvalidOperationException>(() => new GuanceConfig
        {
            DatakitUrl = "http://127.0.0.1:9529",
            RumAppId = "app",
            Cache = new CacheOptions { MaxDiskBytes = 1024 * 1024 }
        }.Validate());

        Assert.Contains("largest configured batch file", exception.Message, StringComparison.Ordinal);
    }

    [Fact]
    public void GuanceConfig_RejectsAnUploadBurstThatCannotFitOneBatch()
    {
        var exception = Assert.Throws<InvalidOperationException>(() => new GuanceConfig
        {
            DatakitUrl = "http://127.0.0.1:9529",
            RumAppId = "app",
            Upload = new UploadOptions { BurstBytes = 512 * 1024 }
        }.Validate());

        Assert.Contains("largest configured upload batch", exception.Message, StringComparison.Ordinal);
    }

    [Fact]
    public async Task CacheDiagnostics_ReportAllocationAwareSharedUsage()
    {
        var config = new GuanceConfig
        {
            DatakitUrl = "http://127.0.0.1:9529",
            RumAppId = "app",
            CacheDirectory = Path.Combine(Path.GetTempPath(), "guance-cache-diagnostics", Guid.NewGuid().ToString("N")),
            Cache = new CacheOptions { MaxBatchItems = 1 }
        };
        var queue = new BatchFileRumQueue(config);
        await using var client = new GuanceClient(config, queue, new RetryRumTransport());

        client.AddAction("cached", "custom", TimeSpan.FromMilliseconds(1));
        await client.FlushAsync();
        var snapshot = client.GetCacheDiagnosticsSnapshot();

        Assert.True(snapshot.AllocatedBytes > 0);
        Assert.True(snapshot.FileCount > 0);
        Assert.Equal(config.Cache.MaxDiskBytes, snapshot.MaxDiskBytes);
        Assert.InRange(snapshot.UsageRatio, 0, 1);
    }
    ...
    private static GuanceConfig CreateTestConfig() => new()
    {
        DatakitUrl = "http://127.0.0.1:9529",
        RumAppId = "app",
        ServiceName = "desktop-service",
    ...
    private sealed class MemoryRumQueue : IRumQueue
    {
        ...
        public Task<QueueBatch<QueuedRumEvent>?> AcquireAsync(CancellationToken cancellationToken) =>
            TestQueueLease.AcquireAsync(Items, item => item.Line.Length, cancellationToken);
    ...
    private sealed class DelayedRumQueue : IRumQueue
    {
        ...
        public async Task<BatchAppendResult> EnqueueAsync(string line, CancellationToken cancellationToken)
        {
