...
public sealed class LogInputTests
{
    ...
    public async Task AddLog_UploadsLoggingLineWithCurrentRumContext()
    {
        ...
        await using var client = new RumClient(new RumConfig
        {
    ...
    {
        var requests = new List<CapturedRequest>();
        await using var client = CreateClient(requests, new RumLogConfig());

        client.AddLog("not collected", RumLogStatus.Info);
        await client.FlushAsync();
    ...
    public async Task AddLog_AppliesLevelFilterAndSamplingIndependently()
    {
        ...
        await using var client = CreateClient(requests, new RumLogConfig
        {
            EnableCustomLog = true,
            SampleRate = 0,
            LevelFilters = new[] { RumLogStatus.Error }
        });

        client.AddLog("filtered", RumLogStatus.Warning);
        client.AddLog("sampled", RumLogStatus.Error);
        await client.FlushAsync();
    ...
    public async Task AddLog_WhenRumLinkIsDisabled_OmitsRumContext()
    {
        ...
        client.AddLog("standalone", RumLogStatus.Info);
        await client.FlushAsync();
    ...
    public async Task AddLog_TruncatesMessageToThirtyKilobytesAtUtf8Boundary()
    {
        ...
        client.AddLog(string.Concat(Enumerable.Repeat("汉", 12_000)), RumLogStatus.Info);
        await client.FlushAsync();
    ...
    public async Task AddLogs_AcceptsEnumAndCustomStatuses()
    {
        ...
        client.AddLogs(new[]
        {
            new RumLogEntry("first", RumLogStatus.Ok),
            new RumLogEntry("second", "audit")
        });
        await client.FlushAsync();
    ...
    public async Task LoggingQueue_RetriesDurablyAcrossClientRestart()
    {
        ...
        await using (var firstClient = CreateClient(
            failedRequests,
            new RumLogConfig { EnableCustomLog = true },
            cacheDirectory,
            HttpStatusCode.ServiceUnavailable))
        {
            firstClient.AddLog("survives restart", RumLogStatus.Error);
    ...
    private static RumClient CreateClient(List<CapturedRequest> requests, RumLogConfig logging)
    {
        return CreateClient(
            requests,
            logging,
    ...
    private static RumClient CreateClient(
        ...
        RumLogConfig logging,
        string cacheDirectory,
        HttpStatusCode statusCode)
    {
        return new RumClient(new RumConfig
        {
            DatakitUrl = "http://127.0.0.1:9529",
            RumAppId = "app-id",
            Env = "local",
