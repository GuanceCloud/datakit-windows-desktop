...
builds the sample C++ application. `preload:build` uses esbuild to bundle the
...
The sample launcher maps these values to the application native process:
...
Both verification paths assert that Browser RUM reaches the same SDK Handle
created by `native-host/main.cpp`. `scripts/start.ps1` only coordinates the two
sample processes; a production application should use its own lifecycle.
