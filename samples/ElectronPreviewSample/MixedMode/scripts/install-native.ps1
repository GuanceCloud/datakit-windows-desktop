...
$null = & (Join-Path $repositoryRoot "build\prepare-vcpkg-registry-port.ps1") `
    -ReleaseTag "vcpkg_0.1.0-alpha.4" `
    -SourceSha512 ("0" * 128) `
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Mixed-mode Native SDK installation failed." }
} finally {
